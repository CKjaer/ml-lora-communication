...
if __name__ == "__main__":
        ...
        test_df_a = pd.read_csv(os.path.join(data_folder, filename))
        test_df_a.rename(columns={test_df_a.columns[0]: 'SNR'}, inplace=True)
        print(filename)
        print(test_df_a)

        filename = "testbatch.csv"
        test_df_b = pd.read_csv(os.path.join(data_folder, filename))
        test_df_b.rename(columns={test_df_b.columns[0]: 'SNR'}, inplace=True)
        print(filename)
        print(test_df_b)

        filename = "testexp.csv"
        test_df_e = pd.read_csv(os.path.join(data_folder, filename))
        test_df_e.rename(columns={test_df_e.columns[0]: 'SNR'}, inplace=True)
        print(filename)
        print(test_df_e)

        rate_params = test_df_e.columns[1:]
        snr_vals = test_df_e.iloc[:, 0]
        ...
        for i, rate_param in enumerate(rate_params):
            ...
            ax.plot(
                snr_vals,
                test_df_a[rate_param],
                marker="s",
                label=f"CNN, autoscaled, λ={rate_param}",
                color="blue",
                linestyle='dotted'
            )  
            ...
            ax.plot(
                snr_vals,
                test_df_b[rate_param],
                marker="s",
                label=f"CNN, batch scaled, λ={rate_param}",
                color="red",
                linestyle='dashed'
            )
            ...
            ax.plot(
                snr_vals,
                test_df_e[rate_param],
                marker="s",
                label=f"CNN, expected, λ={rate_param}",
                color="green",
                linestyle='dashdot'
            )
